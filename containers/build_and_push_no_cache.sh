...
if [ -n "$BUILD_ARG" ]; then
    eval "docker build --no-cache $BUILD_ARG -f ${CURRENT_DIRECTORY}/$DOCKERFILE_PATH -t ${REPOSITORY_NAME} ."
else
    docker build --no-cache -f "${CURRENT_DIRECTORY}/$DOCKERFILE_PATH" -t "${REPOSITORY_NAME}" .
fi
